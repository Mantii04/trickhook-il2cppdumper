// Compact AES-128-CBC decrypt — no padding, len must be multiple of 16.
#include "aes.h"
#include <string.h>

static const uint8_t sbox[256] = {
};

static const uint8_t rsbox[256] = {
};

static const uint8_t Rcon[11] = {0x8d,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36};

static void key_expansion(const uint8_t* key, uint8_t* rk) {
    memcpy(rk, key, 16);
    uint8_t t[4];
    for (int i = 4; i < 44; i++) {
        memcpy(t, rk + (i - 1) * 4, 4);
        if (i % 4 == 0) {
            uint8_t tmp = t[0];
            t[0] = sbox[t[1]] ^ Rcon[i / 4];
            t[1] = sbox[t[2]];
            t[2] = sbox[t[3]];
            t[3] = sbox[tmp];
        }
        rk[i*4+0] = rk[(i-4)*4+0] ^ t[0];
        rk[i*4+1] = rk[(i-4)*4+1] ^ t[1];
        rk[i*4+2] = rk[(i-4)*4+2] ^ t[2];
        rk[i*4+3] = rk[(i-4)*4+3] ^ t[3];
    }
}

static void add_rk(uint8_t* s, const uint8_t* rk) {
    for (int i = 0; i < 16; i++) s[i] ^= rk[i];
}

static void inv_shift_rows(uint8_t* s) {
    uint8_t t;
    t=s[13];s[13]=s[9];s[9]=s[5];s[5]=s[1];s[1]=t;
    t=s[2];s[2]=s[10];s[10]=s[2];
    t=s[14];s[14]=s[6];s[6]=s[14];
    t=s[3];s[3]=s[7];s[7]=s[11];s[11]=s[15];s[15]=t;
}

static uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        uint8_t hi = a & 0x80;
        a <<= 1;
        if (hi) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

static void inv_mix_columns(uint8_t* s) {
    for (int i = 0; i < 4; i++) {
        uint8_t* c = s + i*4;
        uint8_t a = c[0], b = c[1], cc = c[2], d = c[3];
        c[0] = gmul(a,0x0e) ^ gmul(b,0x0b) ^ gmul(cc,0x0d) ^ gmul(d,0x09);
        c[1] = gmul(a,0x09) ^ gmul(b,0x0e) ^ gmul(cc,0x0b) ^ gmul(d,0x0d);
        c[2] = gmul(a,0x0d) ^ gmul(b,0x09) ^ gmul(cc,0x0e) ^ gmul(d,0x0b);
        c[3] = gmul(a,0x0b) ^ gmul(b,0x0d) ^ gmul(cc,0x09) ^ gmul(d,0x0e);
    }
}

static void decrypt_block(const uint8_t* in, uint8_t* out, const uint8_t* rk) {
    uint8_t s[16];
    memcpy(s, in, 16);
    add_rk(s, rk + 40*4);
    for (int r = 9; r >= 1; r--) {
        inv_shift_rows(s);
        for (int i = 0; i < 16; i++) s[i] = rsbox[s[i]];
        add_rk(s, rk + r*16);
        inv_mix_columns(s);
    }
    inv_shift_rows(s);
    for (int i = 0; i < 16; i++) s[i] = rsbox[s[i]];
    add_rk(s, rk);
    memcpy(out, s, 16);
}

void aes128_cbc_decrypt(const uint8_t* key, const uint8_t* iv,
                        const uint8_t* in, uint8_t* out, size_t len) {
    uint8_t rk[176];
    key_expansion(key, rk);
    uint8_t prev[16]; memcpy(prev, iv, 16);
    for (size_t off = 0; off + 16 <= len; off += 16) {
        uint8_t blk[16];
        decrypt_block(in + off, blk, rk);
        for (int i = 0; i < 16; i++) out[off + i] = blk[i] ^ prev[i];
        memcpy(prev, in + off, 16);
    }
}
